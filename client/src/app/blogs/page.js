...
export default function BlogList() {
  ...
  const fetchBlogs = async () => {
    try {
      setLoading(true);
      const res = await fetch("https://blog-editor-xxvi.onrender.com/api/blogs");
  ...
  const handlePublish = async (id) => {
    try {
      setLoading(true);
      console.log(id);
      const res = await fetch(`https://blog-editor-xxvi.onrender.com/api/blogsUpdate/${id}`, {
        method: "POST",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`https://blog-editor-xxvi.onrender.com/api/blogs/${id}`, {
        method: "DELETE",
      });
      ...
      <section className="mb-8">
        ...
        <ul>
          {published.map((blog) => (
            ...
            >
              <h3 className="font-bold text-lg text-green-900">{blog.title}</h3>
